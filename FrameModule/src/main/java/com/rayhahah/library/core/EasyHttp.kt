...
class EasyHttp {
     ...
     * 异步加载数据
     */
    inline fun <reified T : Any> go(noinline success: (data: T) -> Unit, noinline failed: (call: Call, exception: Exception) -> Unit) {
        go<T>(success, failed, { value, total -> })
    }
    ...
    inline fun <reified T : Any> go(noinline success: (data: T) -> Unit,
        ...
        if(BuildConfig.DEBUG){
            Log.map(head)
            val mapLog = HashMap<String,String>()
            mapLog.put("requestUrl",url)
            mapLog.putAll(params.data)
            Log.map(mapLog)
        }
        when (type) {
            TYPE.METHOD_GET ->
